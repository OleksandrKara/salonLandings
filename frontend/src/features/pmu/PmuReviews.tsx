...
export function PmuReviews() {
  return (
    <section style={styles.section}>
      <div style={styles.eyebrow}>Loved in San Diego</div>

      <div style={styles.summaryCard}>
        <GoogleLogo size={34} />
        <div style={{ flex: 1, lineHeight: 1.15 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 30, color: "var(--color-ink)" }}>
              {PMU_RATING.score}
            </span>
            <span style={{ color: "var(--color-gold)", fontSize: 16, letterSpacing: 1 }}>★★★★★</span>
          </div>
          <div style={{ fontSize: 12.5, color: "var(--color-muted-2)", marginTop: 3 }}>
            Based on {PMU_RATING.count} Google reviews
          </div>
        </div>
