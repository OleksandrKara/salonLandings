...
import { PMU_GALLERY_INITIAL_COUNT, PMU_GALLERY_SLIDES } from "@/data/pmuCopy";

export function PmuResultsCarousel() {
  const trackRef = useRef<HTMLDivElement | null>(null);
  const [slideIdx, setSlideIdx] = useState(0);
  const [lightboxIdx, setLightboxIdx] = useState<number | null>(null);
  const [expanded, setExpanded] = useState(false);
  const visibleSlides = expanded ? PMU_GALLERY_SLIDES : PMU_GALLERY_SLIDES.slice(0, PMU_GALLERY_INITIAL_COUNT);
  const hiddenCount = PMU_GALLERY_SLIDES.length - PMU_GALLERY_INITIAL_COUNT;
  ...
  return (
    <section id="pmu-results" style={styles.section}>
      <div style={{ padding: "0 22px" }}>
        <div style={styles.eyebrow}>Real Results</div>
        <h2 style={styles.title}>See the transformation.</h2>
        <p style={styles.subtitle}>Real client healed results. Swipe through — tap any photo to enlarge.</p>
      </div>
        ...
        {visibleSlides.map((slide, i) => (
          <div key={slide.id} style={styles.slide}>
            <div style={styles.slideImageWrap}>
              <img src={slide.src} alt={slide.caption} style={styles.slideImage} loading="lazy" decoding="async" />
      ...
      {lightboxIdx !== null ? (
        <Lightbox
          ...
          onPrev={() => setLightboxIdx((i) => (i! - 1 + PMU_GALLERY_SLIDES.length) % PMU_GALLERY_SLIDES.length)}
          onNext={() => setLightboxIdx((i) => (i! + 1) % PMU_GALLERY_SLIDES.length)}
        />
      ) : null}
    </section>
  );
...
function Lightbox({ index, onClose, onPrev, onNext }: { index: number; onClose: () => void; onPrev: () => void; onNext: () => void }) {
  const slide = PMU_GALLERY_SLIDES[index];
  return (
