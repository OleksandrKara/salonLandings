...
app already reads from at runtime. Run inside the backend container:
    ...
    docker compose exec backend python -m app.scripts.manage_landing_variants add \\
        --slug mani --key holiday-gold --name "Holiday Gold" \\
        --headline "Holiday Glam, Booked in Seconds" \\
        --cta "Claim Your Holiday Slot" \\
        --accent-color "#B8860B"

A variant created with a --key is reachable via a deep link (mani.akluxnails.com/?v=holiday-gold)
regardless of its weight — set --weight above 0 only if it should also be eligible for the
random A/B pool shown to regular organic/direct traffic.
"""
...
async def cmd_add(args: argparse.Namespace) -> None:
    ...
    reach = f"deep link: ?v={args.key}" if args.key else "no key — only reachable via the random A/B pool"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_add.add_argument("--key", default=None, help="Campaign key for ?v=<key> deep-links, e.g. 'holiday-gold'")
